...
export default function ProfileSetupPage() {
  ...
  const isInCooldownPeriod = () => {
    ...
    return (
      ...
      new Date().getTime() - cooldownPeriod
    );
  };
      ...
      <div className="w-full space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Your alias must be at least 5 characters, start with a letter, and
            contain only letters, numbers, underscores, or hyphens. This will be
            your unique identifier on the platform.
            {profile?.aliasUpdatedAt && (
              <span className="block mt-1">
                <strong>Note:</strong> You can only update your alias once every
                7 days.
                {isInCooldownPeriod() && (
                  <span className="text-amber-600">
                    {" "}
                    Your next update will be available after{" "}
                    {getNextUpdateDate()}.
                  </span>
                )}
              </span>
        ...
        {profile?.alias && message && (
          <p className="text-center text-sm text-gray-600 mt-4">
            Your profile has been updated successfully! Continue exploring the
            platform with your new alias.
          </p>
        )}
      </div>
    </div>
