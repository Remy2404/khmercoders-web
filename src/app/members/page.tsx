...
import { ArrowRight, Github, Globe, Linkedin, Twitter } from 'lucide-react';
...
export default function MembersPage() {
  return (
    <main className="container mx-auto px-4 pb-20">
      ...
      <section className="mt-24 bg-secondary rounded-lg p-8 md:p-12 text-center">
    ...
    <div className="bg-secondary rounded-lg overflow-hidden border border-border flex flex-col h-full">
      <div className="relative">
        ...
        <div className="absolute -bottom-12 left-1/2 -translate-x-1/2">
          <div className="h-24 w-24 rounded-full overflow-hidden border-4 border-border relative">
            <Image
      ...
      <div className="p-6 pt-16 flex-grow flex flex-col">
        ...
        {member.badges && member.badges.length > 0 && (
          <div className="flex flex-wrap justify-center gap-2 mb-4">
            {member.badges.map((badge, index) => (
              <Badge key={index} variant="outline" className="border-orange-500/30 text-orange-500">
                {badge}
              </Badge>
            ))}
          </div>
        ...
        <p className="text-muted-foreground text-center mb-6">{member.shortBio}</p>
