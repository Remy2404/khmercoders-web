...
export function UserLevelBadge({ level }: { level: UserLevel }) {
  if (level === UserLevel.SuperAdmin) {
    return (
      <span className="inline-flex  font-mono py-0.5 px-2 rounded text-xs bg-orange-500 items-center gap-1 text-white">
  ...
  if (level === UserLevel.Premium) {
    return (
      <span className="inline-flex font-mono py-0.5 px-2 rounded text-xs bg-purple-500 items-center gap-1 text-white">
